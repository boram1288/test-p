...
현재 프로젝트 자체의 근거는 후보 구조와 동작 설명뿐이다. PoC, 시험 결과, 계산 결과와 운영 관찰값은 없다.
따라서 평가 근거 수준은 모두 `구조적 추론` 또는 `확인 필요`다.
필수 gate는 실제 검증 전까지 `확인 필요`로 판정하고, KPI 값과 별점 구간이 없는 품질속성에는 확정 별점을 부여하지 않는다.
사용자가 요청한 구조적 예상 점수는 6.7절에서 `구조 휴리스틱 예상 별점`으로 분리한다.
...
PERF-07은 문제 요구 추적용 ID이며, M-01d는 6.5절에서 사용하는 KPI 정의의 직접 출처다.
...
별점 구간과 후보별 KPI 값이 모두 없으므로 이 표에는 KPI 기반 별점을 부여하지 않는다.
6.7절의 예상 별점은 별도의 구조 휴리스틱이며 이 표의 `별점 TBD`를 대체하지 않는다.
전체 총점은 계산하지 않는다.
...
이 절은 실측 전에 구조적 방향을 비교해 달라는 사용자 요청에 따라 작성한다.
`후보 구조 품질 평가 규칙` 7.2절이 허용하는 구조적 예상 점수이며 KPI 기반 확정 별점이 아니다.

| 예상 별점 | 구조 휴리스틱 기준 |
|---|---|
| 잠정 ★★★ | 후보를 가르는 메커니즘과 필요한 완화책이 현재 구조에 포함되고, 직접 비교 가능한 사례나 분석이 이를 뒷받침함 |
| 잠정 ★★☆ | 구조적 방향과 유사 사례는 있으나 프로젝트별 자동화·HA·운영 검증 또는 직접 비교 근거가 추가로 필요함 |
| 잠정 ★☆☆ | 현재 구조가 공유 핵심 경로나 장애 집중 위험을 만들고, 이를 줄일 완화 메커니즘이 후보 설명에 없음 |

모든 점수는 `실측 없음`, `구조 휴리스틱`, `KPI 확정 아님`을 전제로 한다.
외부 사례는 메커니즘의 실현 가능성과 반례를 확인하는 자료이며 점수나 KPI 수치의 출처가 아니다.
보안성, 최신 파일 복구성, 저장 상태 정확성과 고아 자원 회수는 pass/fail gate이므로 예상 별점을 부여하지 않는다.

#### 6.7.2 사례·웹·논문 조사 근거

| 구분 | 조사 자료 | 확인한 구조적 사실 | 이번 평가에 사용한 범위와 한계 |
|---|---|---|---|
| 공식 사례 | [Kubernetes Persistent Volumes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/) | PV는 Pod와 독립적인 수명을 가지며 동적 provisioning, 일대일 binding과 단일 Pod 쓰기 제약을 지원함 | 후보 B의 수명 분리·자동화·단일 writer 실현 가능성만 뒷받침함. pVM의 성능값을 증명하지 않음 |
| 공식 사양 | [Container Storage Interface](https://github.com/container-storage-interface/spec/blob/master/csi.proto) | Controller publish/unpublish와 Node stage/publish 등 볼륨 연결 단계가 명시됨 | 후보 B의 연결 단계와 상태가 추가된다는 근거로만 사용함. 각 단계의 지연은 알 수 없음 |
| 설계 사례 | [Kubernetes CSI 설계 문서](https://github.com/kubernetes/design-proposals-archive/blob/main/storage/container-storage-interface.md) | out-of-tree driver로 코어 결합을 줄이지만 provision/delete와 attach/detach를 담당할 외부 컴포넌트가 필요함 | 후보 B도 코어 수정 없는 확장이 가능하다는 반례임. 과거 설계 문서이므로 현재 프로젝트 성공률로 전이하지 않음 |
| 공식 구현 사례 | [Kata Containers Storage](https://github.com/kata-containers/kata-containers/blob/main/docs/design/architecture/storage.md) | VM별 `virtiofsd`, block device hotplug와 guest의 직접 block I/O 경로가 실제 구현됨 | Workload별 서비스·장치가 가능하지만 자원과 준비 단계가 생긴다는 사례임. Kata와 pVM의 수치를 동일시하지 않음 |
| 원 논문 | [The Google File System](https://static.googleusercontent.com/media/research.google.com/en/us/archive/gfs-sosp2003.pdf) | 단일 master가 namespace를 통합하지만 데이터는 master를 우회하며, log 복제와 shadow master로 장애를 완화함 | 후보 A의 공용 namespace는 가능한 구조지만 공용 I/O와 HA 완화책이 없으면 병목·장애 집중 위험이 남는다는 반례로 사용함 |
| 원 논문 | [Ceph: A Scalable, High-Performance Distributed File System](https://www.usenix.org/event/osdi06/tech/full_papers/weil/weil.pdf) | metadata와 data를 분리하고 client가 OSD와 직접 I/O하며 MDS·OSD를 분산함 | 후보 B의 제어·데이터 경로 분리 방향을 뒷받침함. Ceph의 분산·복제 결과를 Workload별 단일 볼륨에 전이하지 않음 |
| 원 논문·운영 사례 | [Firecracker: Lightweight Virtualization for Serverless Applications](https://www.usenix.org/conference/nsdi20/presentation/agache) | 전용 microVM의 시작·메모리 overhead가 작게 최적화되어 대규모 운영된 사례가 있음 | 전용 실행 단위의 cold start 비용이 항상 크다는 가정을 반박함. 저장 volume overhead를 다루지 않으므로 M-02c 근거로 사용하지 않음 |

#### 6.7.3 예상 별점 결과

| KPI | 후보 A 예상 별점 | 후보 A 근거·반례 | 후보 B 예상 별점 | 후보 B 근거·반례 | 검증 계획 |
|---|---|---|---|---|---|
| M-01d 저장 재연결 cold start | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | 상시 공용 서비스의 namespace 재연결만 필요해 방향상 유리함. 직접 지연 비교 사례와 실측은 없음 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | CSI·Kata 사례처럼 attach·stage·mount와 서비스 준비가 추가되지만 표준 자동화가 가능함 | 같은 pVM 이미지와 부하에서 생성 시작부터 서비스 준비까지 p95 측정 |
| M-02c 영구 저장 overhead | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | 서비스·정책 자원을 공유해 Workload당 증가량이 낮을 가능성이 있음. 공용 metadata와 격리용 quota 비용은 미정 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | Workload별 volume·filesystem·서비스가 필요함. Kata 사례는 전용 장치 구성이 가능함을 보이지만 저장 overhead 수치는 제공하지 않음 | Workload 수와 파일 수를 늘리며 volume·metadata·프로세스 자원을 분리 측정 |
| M-06c 비관련 Workload 영향률 | 잠정 ★☆☆ — 구조 휴리스틱, KPI 확정 아님 | 공용 서비스가 모든 파일 I/O의 공유 핵심 경로이며 quota·partition·복제 방식이 후보에 없음 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | 파일 I/O 경계는 Workload별이지만 Framework 볼륨 관리자와 물리 저장 매체는 공용 장애점으로 남음 | 공용 서비스·특정 volume·볼륨 관리자 장애를 각각 주입해 비관련 Workload 중단률 측정 |
| M-06d 저장 서비스 가용성·MTTR | 잠정 ★☆☆ — 구조 휴리스틱, KPI 확정 아님 | 공용 서비스 중단이 전체 Workload 응답 중단으로 이어지며 GFS식 복제·shadow·data bypass가 정의되지 않음 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | pVM 저장 서비스 장애는 Workload별로 복구할 수 있으나 공용 볼륨 관리자 장애 복구는 미정 | 동일 장애 횟수·관찰 시간으로 Availability와 MTTR 측정 |
| M-08b 신규 대상 통합 리드타임 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | 정책과 namespace 등록 중심이라 방향상 단순하지만 5인일 이내 사례나 프로젝트 실측이 없음 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | 볼륨 자동화 작업이 추가되지만 Kubernetes 동적 provisioning 사례처럼 코어 밖 자동화가 가능함 | 같은 완료조건과 숙련도로 신규 저장정책 1종의 실제 작업 인일 측정 |
| M-08c 설정·plugin 기반 통합 성공률 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | 정책 데이터 경계는 있으나 안정적인 plugin 계약과 반복 통합 결과가 없음 | 잠정 ★★☆ — 구조 휴리스틱, KPI 확정 아님 | CSI는 out-of-tree 확장 가능성을 보여주지만 현재 Framework의 volume plugin 경계는 미구현 | 신규 Workload 유형을 반복 추가해 코어 무수정 성공률 측정 |

예상 별점의 동점은 구조적 차이가 없다는 의미가 아니다.
M-01d·M-02c·M-08b는 후보 A가 방향상 유리하지만 직접 비교 사례와 실측이 없어 두 후보를 같은 잠정 구간에 둔다.
M-06c·M-06d는 후보 A의 공용 I/O 장애 집중과 후보 B의 공용 제어부 잔존을 함께 반영해 한 단계만 차이를 둔다.

#### 6.7.4 사용 제한

- 예상 별점은 KPI 목표 충족 여부, 확정 평가와 후보 선택의 근거로 사용하지 않는다.
- 외부 시스템의 지연, 메모리, 처리량과 가용성 수치를 이 프로젝트의 예상값으로 전이하지 않는다.
- 후보 A에 data bypass, 복제, shadow 서비스나 Workload별 partition을 추가하면 M-06c·M-06d를 다시 평가한다.
- 후보 B의 볼륨 관리자 이중화와 attach 상태 복구가 정의되지 않으면 M-06c·M-06d를 상향하지 않는다.
- PoC 이후에는 6.5절의 KPI 구간을 먼저 승인하고, 이 예상 별점을 실제 KPI 별점으로 교체한다.
...
- 구조 휴리스틱 예상 별점은 가용성에서만 후보 B를 한 단계 높게 평가하고, 나머지 KPI는 직접 근거 부족으로 같은 잠정 구간에 둔다.
