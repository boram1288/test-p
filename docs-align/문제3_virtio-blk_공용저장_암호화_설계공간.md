...
> 비신뢰 Host가 제공한 공용 UFS 공간을 pVM이 `virtio-blk`로 사용할 때, pVM 세대가 바뀌어도 같은 Workload가 최신 자료를 이어 쓰도록 해야 한다. 동시에 평문 키와 평문 자료가 Host에 노출되지 않아야 하며, Host가 자료나 위치표를 바꾸거나 이전 상태로 되돌리면 이를 알아내야 한다. 이를 위해 암호 처리를 어느 실행 위치와 어느 저장 계층에 둘지, EL2·TEE·Host·pVM의 책임을 어떻게 나눌지 정해야 한다.
...
| 선택 | 좋아지는 점 | 부담되는 점 |
|---|---|---|
| 암호 처리를 TEE에 모음 | 키 노출 범위를 줄일 수 있다. | TEE 호출 횟수, 버퍼 복사와 실행 영역 전환이 늘 수 있다. |
| 암호 처리를 pVM 커널에 둠 | 응용프로그램을 바꾸지 않아도 된다. | 커널 변경 범위와 장애 영향이 커진다. |
| 파일 단위로 처리 | 일부 자료만 갱신하기 쉽다. | 파일 크기·시간 등 일부 파일시스템 정보가 보일 수 있다. |
| 블록 단위로 처리 | 파일시스템 전체를 가릴 수 있다. | 블록 위치표, 쓰기 순서와 비정상 종료 복구가 복잡해진다. |
| RPMB에 자주 확정 | 손실 가능한 변경량이 줄어든다. | RPMB 쓰기 횟수와 저장 지연이 늘 수 있다. |
| 공용 물리 공간 사용 | 저장 낭비를 줄인다. | Host가 관리하는 할당표를 신뢰할 수 없으므로 TEE가 확인할 수 있는 자료 목록과 최신 상태가 필요하다. |
