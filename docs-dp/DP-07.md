...
- 데이터 흐름: 없음(제어 동작 위주). 복구 중 정상 pVM 사이의 프레임 데이터 흐름은 유지된다.
- 제어 흐름: pVM별 복구 관리자가 장애를 감지해 해당 pVM의 자원만 회수·재기동하고, 남아 있는 channel·HW lease·TEE 세션·저장 연결에 새 pVM 인스턴스를 다시 연결한다.
- 신뢰/비신뢰 주체: Host는 비신뢰. pVM별 복구 관리자는 Framework 신뢰 경계 안에 있다.
...
- 신뢰/비신뢰 주체: Host는 비신뢰. 파이프라인 복구 관리자는 Framework 신뢰 경계 안에 있다.
...
Cam -down-> Chan : 프레임 (데이터 흐름, 실선, 복구 중에도 유지)
