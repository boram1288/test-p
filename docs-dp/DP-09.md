...
- 신뢰/비신뢰 주체: Host의 Framework 코어는 비신뢰 관리 주체다. DP-02 결과를 사용하는 보호 검증 주체가 Workload 신원과 요청 권한을 최종 확인한다.
...
- 신뢰/비신뢰 주체: Host의 Adapter와 Framework 코어는 비신뢰 관리 주체다. DP-02 결과를 사용하는 보호 검증 주체가 변환 뒤에도 Workload 신원과 요청 권한을 최종 확인한다.
...
package "보호 검증 경계" #E8F6EF {
  [Workload 신원/권한 최종 확인] as Verify
}
...
Core ..> Verify : canonical 요청 검증 (제어 흐름, 점선)
