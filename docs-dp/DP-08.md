...
- 신뢰/비신뢰 주체: Host는 비신뢰. CPU 배치를 결정하는 Framework는 신뢰 경계 안에 있다. 보안 경계 자체는 바뀌지 않는다.
...
- 신뢰/비신뢰 주체: Host는 비신뢰이지만 스케줄링 판단을 맡는다. 우선순위·quota 값을 정하는 Framework는 신뢰 경계 안에 있다. 보안 경계 자체는 바뀌지 않는다.
...
CamCPU -down-> CamCPU : 실행 (데이터 흐름, 실선, Host 작업 배제)
