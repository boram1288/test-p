...
- 선행/연관 DP: DP-01(pVM 실행 흐름의 격리 경계)에서 pVM을 생성·삭제할 때 이 DP가 정한 저장 공간을 연결하거나 분리해야 한다. DP-02(Workload 최종 검증 위치)에서 확인한 안정적인 Workload 식별자로 이전 저장 공간을 다시 찾는다. DP-05는 TEE 명령의 전달 경로를 정하고, 이 DP는 TEE가 암호화한 파일의 보관 주체를 정한다.
