...
- DP-07은 이 DP에서 정한 실행 경계 안에서 장애 후 회수와 재구성 범위를 정한다. DP-08은 CPU 공유 경계를 별도로 정한다.
...
- 자원 소유/회수: 각 worker가 논리적 원장을 소유한다. 공용 제어기가 worker hang을 감지하고 DP-07의 회수 절차를 시작한다.
...
4. 공용 제어기는 DP-07에서 정한 범위로 자원 회수를 요청한다.
...
4. 제어 프로세스는 실패 프로세스를 격리하고 DP-07의 회수 절차를 시작한다.
...
| 회수 연계 | 장애 뒤 DP-07의 원장/실자원 대조 수행 | 잔여 자원은 DP-07과 AVL-04 기준으로 판정 |
