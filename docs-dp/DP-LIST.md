...
- DP-07은 DP-03 계열의 공유 버퍼, DP-04의 HW 권한, DP-05의 TEE 세션, DP-06의 저장 연결과 DP-10의 미완료 frame을 장애 뒤 회수하는 범위를 사용한다.
- DP-08은 DP-01에서 정한 pVM 실행 경계에 CPU 실행 자원을 배치한다. CPU 예약량은 DP-01의 하위 결정으로 남기고 런타임 공유 경계만 DP-08에서 정한다. DP-07의 복구 작업이 정상 pVM의 CPU 실행을 방해하지 않도록 AVL-05 예산도 함께 맞춘다.
