...
- 문제 상황:
    - 현재 구조: Camera pVM과 AI pVM 사이의 프레임 전달 요청은 Host가 중계하고, EL2가 버퍼 소유자, 수신자, 전송 식별자와 권한 회수 상태를 최종 확인한다.
    - 보안 문제: Host 중계 과정에서 전달 시점, 송신자와 수신자 같은 부가 정보가 Host에 노출된다. 권한 전환을 잘못 처리하면 격리 메모리의 원본 데이터까지 노출될 수 있다(SEC-01, 부가 정보 노출 TBD).
    - 전달 지연 문제: 프레임 데이터는 복사하지 않더라도 Host 중계와 EL2 호출 단계가 매번 실행되어 전달 지연이 늘어난다(PERF-02).
    - API 변경 문제: 버퍼 전달 기능이 Host의 중계 API에 묶이면 Framework를 변경할 때 기존 Workload API도 함께 바뀔 수 있다(EXT-07).
    - 관련 요구: FR-05
...
- 문제 상황:
    ...
    - 권한 전환 문제: 권한 회수, 남은 데이터 삭제와 다음 주체 권한 부여가 정확한 순서로 끝나지 않으면 두 주체의 권한이 겹치거나 할당되지 않은 메모리로 DMA가 접근할 수 있다(SEC-02, SEC-03).
    ...
    - 정책 변경 문제: 순서 결정을 보호 영역으로 옮기면 우선순위나 예약 정책을 바꿀 때 EL2 코드까지 수정해야 한다(변경 용이성 TBD).
    - 관련 요구: FR-04
...
- 문제 상황:
    ...
    - 영구 저장 문제: 저장 서비스와 파일시스템을 pVM 안에 두면 pVM 삭제 시 암호화된 파일도 함께 사라질 수 있다(가용성 TBD).
    - 식별 문제: 새로 생성된 pVM은 이전 pVM과 식별자가 달라 같은 파일을 찾지 못할 수 있다(가용성 TBD).
    - 보안 문제: 일반 영역의 파일시스템이 침해되면 저장 파일이 복사되거나 바뀔 수 있다. TEE 밖으로 평문이나 키가 나가는 경로가 있으면 해당 정보도 Host에 노출된다(SEC-05).
    - 재연결 지연 문제: pVM을 다시 만들 때 이전 저장 공간을 찾고 연결하는 단계가 추가되어 첫 프레임 처리까지의 시간이 늘어난다(PERF-07).
    - Workload 추가 문제: Workload마다 별도 저장 구조를 추가하면 Framework 코어나 배포 구성을 바꿔야 하고 통합 시간이 늘어날 수 있다(EXT-01, EXT-03).
    - 자원 사용 문제: Workload마다 저장 서비스와 저장 공간을 따로 두면 메모리, 프로세서와 저장 공간 사용량이 늘어난다(자원 효율 TBD).
