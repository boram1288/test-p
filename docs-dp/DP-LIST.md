...
- 문제 상황: Secure Storage를 사용하면 TEE가 데이터와 키를 처리하고, 일반 영역의 저장 서비스가 암호화된 파일을 파일시스템에 기록한다. 이 저장 서비스와 파일시스템을 pVM 안에 두면 pVM 삭제 시 암호화된 파일도 함께 사라질 수 있다. 새로 생성된 pVM은 이전 pVM과 식별자가 달라 같은 파일을 찾지 못할 수도 있다. 저장 구조는 Host 파일시스템이 침해되어도 평문과 키가 노출되지 않아야 하고(SEC-05), pVM 재생성 후에도 이전 파일을 사용할 수 있어야 한다(가용성 TBD). 저장 공간 연결은 최초 시작 시간에 포함된다(PERF-07). 신규 Workload는 Framework 코어 수정 없이 추가되어야 하고(EXT-01), 저장 공간 구성까지 포함한 통합 시간도 목표 안에 들어야 한다(EXT-03). 저장 서비스가 사용하는 자원도 줄여야 한다(자원 효율 TBD). 저장 기능을 함께 쓰면 연결 시간, 구성 작업과 자원 사용을 줄일 수 있지만 장애 영향이 넓어진다. Workload마다 나누면 장애 영향은 줄지만 연결 시간, 구성 작업과 자원 사용이 늘어난다. 따라서 암호화된 파일을 보관할 주체를 정해야 한다.
