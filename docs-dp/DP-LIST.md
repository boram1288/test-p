...
### DP-04. HW 사용 순서 관리 위치

- 결정 질문: Camera/AI HW의 사용 순서를 Host가 관리할 것인가, 보호된 중재 pVM이 관리할 것인가?
- 후보 A: Host가 사용 순서를 관리하고 EL2가 권한 변경 수행
- 후보 B: 보호된 중재 pVM이 사용 순서를 관리하고 EL2가 권한 변경 수행
- 책임과 경계: 두 후보 모두 EL2가 S2MPU/DMA 권한 회수, 남은 데이터 삭제와 다음 사용자에 대한 권한 부여를 중간 중첩 없이 수행한다. HW 사용 순서와 요청 대기열만 Host 또는 보호된 중재 pVM에서 관리한다.
- 관련 요구: FR-04
- 정렬 품질속성: 보안성(SEC-02, SEC-03), 성능(PERF-04), 자원 효율(TBD)
- 핵심 트레이드오프: Host가 사용 순서를 관리하면 기존 드라이버와 Host 자원을 다시 사용할 수 있다. 보호된 중재 pVM이 관리하면 침해된 Host가 HW 사용 순서를 조작하기 어려워진다. 대신 중재 pVM이 계속 사용하는 메모리와 프로세서 자원이 필요하고, 프로세스 간 통신 시간이 추가된다.
...
### DP-06. 저장 데이터 암복호화 실행 위치

- 결정 질문: TEE가 키 관리와 대용량 암복호화를 모두 수행할 것인가, TEE는 키만 관리하고 신뢰 상태를 확인한 pVM이 암복호화를 수행할 것인가?
- 후보 A: TEE 완결 암복호화
- 후보 B: TEE 키 관리와 pVM 암복호화 실행 분리
- 책임과 경계: 후보 A는 키와 평문 처리 책임을 TEE 안에 둔다. 후보 B는 TEE가 신뢰 상태를 확인한 pVM에 세션 키를 전달하고, 대용량 암복호화는 pVM에서 수행한다.
- 관련 요구: FR-06
- 정렬 품질속성: 보안성(SEC-05), 성능(PERF-03), 자원 효율(TBD)
- 핵심 트레이드오프: TEE 완결 구조는 키가 존재하는 영역을 TEE로 제한할 수 있다. 대신 암복호화 요청마다 pVM과 TEE 사이의 실행 영역 전환이 발생하고 TEE 자원을 사용한다. pVM 실행 구조는 대용량 처리 시간과 TEE 부하를 줄이지만, pVM 신뢰 상태 확인, 키 회수와 메모리에서의 키 삭제를 추가로 관리해야 한다.

## 4. 선행 및 연관 관계

- DP-01은 다른 pVM 기반 구조를 실행하는 데 먼저 필요하다.
- DP-02에서 확인한 Workload 신원은 DP-05의 호출자 신원 확인과 DP-06의 키 전달에 사용할 수 있다.
- DP-05는 DP-06의 TEE 명령 전달 경로를 정한다.
- DP-01과 DP-02는 같은 최초 시작 시간에 포함되므로 PERF-07의 시간 예산을 나누어 사용해야 한다.
- DP-03과 DP-04는 같은 프레임 처리 시간에 포함되므로 PERF-02와 PERF-04의 시간 예산을 함께 조정해야 한다.
- DP-05와 DP-06은 같은 전체 처리 시간에 포함되므로 PERF-03의 시간 예산을 나누어 사용해야 한다.
...
초안에서 Host가 버퍼 전달 상태를 최종 관리하는 구조와 EL2가 HW 사용 순서까지 관리하는 구조를 보안 기준과 EL2 코드 최소화 관점에서 다시 검토했다.
최종적으로 버퍼 소유권 상태와 HW 권한 변경은 EL2가 최종 관리하도록 고정했다. DP에는 Host를 전달 경로에 포함할지와 HW 사용 순서를 어디에서 관리할지만 남겼다.
