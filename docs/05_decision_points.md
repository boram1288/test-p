...
|:--:|------|------------|:-------:|
| DP1 | 기밀성, 확장성, 가용성을 위한 pVM 관리 골격 설계 | FR-01, FR-02, FR-05, QA-01, QA-03, QA-05, CS-01, CS-02 | — |
| DP2 | 무결성 보장을 위한 보안 Workload 실행 방식 설계 | FR-01, FR-05, QA-01, QA-03, CS-01, CS-02 | DP1 |
| DP3 | 성능(통신 오버헤드)을 위한 zero-copy 보안 채널 구조 설계 | FR-03, FR-04, QA-01, QA-02, QA-04, CS-01, CS-02 | DP2 |
| DP4 | 성능(실시간 처리) 및 자원 효율을 위한 HW IP 중재 구조 설계 | FR-03, QA-01, QA-02, CS-01, CS-02 | DP3 |
| DP5 | GP API Backward Compatibility를 위한 TrustZone 연동 구조 설계 | FR-06, QA-01, QA-02, QA-04, CS-03 | DP4 |
| DP6 | 보안(기밀성)을 위한 Secure Storage 구조 설계 | FR-06, QA-01, QA-02, CS-03 | DP5, DP2 |
...
- **DP3~DP4 (핵심 리스크/성능 메커니즘)**: 대용량 영상 전달과 HW IP 공유는 본 과제의 핵심 기술 리스크이자 성능(QA-02, QA-04)을 좌우하는 결정이므로, 골격 확정 직후 착수하여 조기 PoC로 검증한다. 두 DP는 DMA 버퍼 소유권을 공유하므로 상호 참조하여 정합을 맞춘다.
...
- QA-03: 신규 Workload를 Framework 코어 수정 0 LoC로 수용할 수 있는 골격이어야 함
- QA-05: pVM 장애 시 Host/타 pVM 다운타임 0을 유지하며 자원 회수와 3초 이내 재시작이 가능해야 함
...
- QA-03: 신규 Workload를 코어 수정 없이 패키징/탑재만으로 수용하는 표준 계약이 필요함
...
- QA-02: 프레임 전달이 30fps, E2E 지연 100ms 이내의 실시간 처리 예산 안에 들어야 함
- QA-04: 도메인 간 버퍼 전달 시 memcpy 호출 0회의 zero-copy가 요구됨
...
- QA-02: HW 사용 주체 전환 오버헤드가 30fps 실시간 파이프라인을 해치지 않아야 함
...
- QA-02: TEE 호출이 파이프라인 경로에 포함될 때 실시간 처리 성능을 해치지 않아야 함
- QA-04: pVM→TEE 호출 경로의 통신 오버헤드가 과도하지 않아야 함
...
- QA-02: 저장/복구 경로의 암호화/복호화가 실시간 파이프라인 성능을 해치지 않아야 함
