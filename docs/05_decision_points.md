...
> 본 문서는 `04_architectural_drivers.md` 5.2절의 설계 착수 순서(T-1 ~ T-5 연계 6단계), `99_additional_design_points.md`의 교차(cross-cutting) Decision Point, 추가로 식별한 보안 정책 DP를 통합하여, 아키텍처 설계 착수 전 확정해야 할 **Decision Point**를 정리한다.
>
...
+
남은 DP는 선행 의존성 기준으로 `DP-01`부터 `DP-09`까지 재정렬했다.
...
| DP-06 | 보안 워크로드 실행 방식 설계 | T-연계 | FR-06, FR-07, QA-07, QA-08, CONST-06 | DP-07 |
| DP-07 | 성능(실시간 처리) 및 자원 효율을 위한 HW IP 중재 구조 설계 | T-연계 | FR-03, QA-03, QA-11, CONST-05 | DP-05, DP-04(병행) |
| DP-08 | 성능(통신 오버헤드)을 위한 zero-copy 보안 채널 구조 설계 | T-연계 | FR-04, QA-04, QA-05 | DP-06 |
| DP-09 | 보안(기밀성)을 위한 Secure Storage 구조 설계 | 교차 | QA-01 × FR-08 | DP-03, DP-07 |
...
flowchart TD
    ...
    DP06["DP-06<br/>보안 워크로드 실행 방식 설계"]
    DP07["DP-07<br/>성능(실시간 처리) 및 자원 효율을 위한 HW IP 중재 구조 설계"]
    DP08["DP-08<br/>성능(통신 오버헤드)을 위한 zero-copy 보안 채널 구조 설계"]
    DP09["DP-09<br/>보안(기밀성)을 위한 Secure Storage 구조 설계"]
