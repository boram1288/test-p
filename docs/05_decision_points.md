...
- QA-04: 다양한 Workload를 Framework 수정 없이 수용할 수 있어야 함
- QA-06: pVM 장애가 Host/타 pVM으로 전파되지 않고 격리되어야 함
- CS-01: Android 스택 의존 없이 Linux 네이티브로 동작해야 함
- CS-02: pKVM 커널 패키지 제약 안에서 pVM 관리 기능을 배치해야 함
**선행 DP Candidate**
...
- pVM 장애 시 자원이 안전하게 회수/재시작되지 않으면 Host/타 pVM의 가용성(QA-06)이 깨진다.
...
**산출근거 Driver**
- FR-01: pVM 시작/정지 흐름과 Workload 실행/중지 절차가 맞물려야 함
- FR-05: 보안 Workload는 서명 검증 후 동적으로 탑재되어야 함
- QA-01: Host 침해 상황에서도 승인되지 않은 Workload 실행을 막아야 함
- QA-04: 신규 Workload를 표준 실행/로딩 계약으로 수용할 수 있어야 함
- CS-01: Linux 네이티브 환경에서 Workload 실행 방식을 정의해야 함
- CS-02: pKVM 제약 안에서 검증/로딩 책임을 배치해야 함
**선행 DP Candidate**
...
**산출근거 Driver**
- FR-03: Camera/AI HW 공유 과정의 버퍼 소유권과 데이터 전달 경로가 채널 구조와 정합되어야 함
- FR-04: pVM↔pVM, pVM↔Host 간 데이터를 비신뢰 주체에 노출 없이 전달해야 함
- QA-01: Host 침해 상황에서도 채널 데이터의 기밀성이 유지되어야 함
- QA-02: 공유 메모리/버퍼 접근 권한이 도메인 격리를 깨지 않아야 함
- QA-03: 영상/추론 데이터 전달이 실시간 처리 성능을 만족해야 함
- QA-05: zero-copy 구조로 도메인 간 통신 오버헤드를 줄여야 함
- CS-01: Linux 네이티브 IPC/공유 메모리 제약 안에서 구현되어야 함
- CS-02: pKVM 제약 안에서 채널 권한과 메모리 공유 방식을 배치해야 함
**선행 DP Candidate**
...
**산출근거 Driver**
- FR-03: Camera/AI HW를 Host와 pVM이 공유/할당할 수 있어야 함
- QA-01: HW 사용 중 보호 자산이 Host에 노출되지 않아야 함
- QA-02: HW IP DMA 경로와 접근 권한이 도메인별로 격리되어야 함
- QA-03: HW 공유/전환 오버헤드가 실시간 처리를 해치지 않아야 함
- CS-01: Linux 네이티브 Driver/중재 구조 안에서 구현되어야 함
- CS-02: pKVM 기반 격리 제약 안에서 HW 권한 전환을 설계해야 함
**선행 DP Candidate**
...
**산출근거 Driver**
- FR-06: pVM Workload가 Secure OS ENC/DEC 명령을 전송할 수 있어야 함
- QA-01: TEE 연동 중 키/민감 데이터가 Host에 노출되지 않아야 함
- QA-03: pVM→TEE 호출 경로가 실시간 파이프라인 성능을 해치지 않아야 함
- QA-05: TEE 호출 경로의 통신 오버헤드가 과도하지 않아야 함
- CS-03: 기존 GP API/TEE 기능은 무회귀로 유지해야 함
**선행 DP Candidate**
...
**산출근거 Driver**
- FR-06: Secure OS ENC/DEC 기능을 저장 보호 경로와 연동해야 함
- QA-01: 저장 상태의 모델/영상/추론 데이터 기밀성이 보장되어야 함
- QA-03: 암호화/복호화가 실시간 파이프라인 성능을 해치지 않아야 함
- CS-03: 기존 TrustZone TEE 기능과 GP API 호환성을 유지해야 함
**선행 DP Candidate**
