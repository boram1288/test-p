...
| DP 선정 | DP Candidate | DP 제목 | 선정 근거 |
|:-------:|:------------:|-----------|-------------------------------------|
| **DP1** | DP-C-03 | 기밀성, 확장성, 가용성을 위한 pVM 관리 골격 설계 |  |
| **DP2** | DP-C-04 | 무결성 보장을 위한 보안 Workload 실행 방식 설계 |  |
| **DP3** | DP-C-06 | 성능(통신 오버헤드)을 위한 zero-copy 보안 채널 구조 설계 | Workload 실행 구조가 정해진 뒤 pVM 간 데이터 전달 경로와 채널 권한/버퍼 보호 모델을 먼저 확정한다. |
| **DP4** | DP-C-07 | 성능(실시간 처리) 및 자원 효율을 위한 HW IP 중재 구조 설계 | zero-copy 채널의 버퍼/소유권 가정을 바탕으로 Camera/AI HW 접근 전환과 DMA 격리 경계를 정한다. |
| **DP5** | DP-C-05 | GP API Backward Compatibility를 위한 TrustZone 연동 구조 설계 | HW/채널 경계가 정해진 뒤 기존 TEE/GP API 호출 경로와 후방호환 범위를 확정한다. |
| **DP6** | DP-C-08 | 보안(기밀성)을 위한 Secure Storage 구조 설계 |  |
| **DP7** | DP-C-01 | 시스템의 보안 정책 신뢰를 위한 신뢰 모델 결정 |  |
| **DP8** | DP-C-02 | 접근 통제 일관성을 위한 보안 정책 및 권한 집행 모델 설계 |  |
