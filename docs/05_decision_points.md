...
| DP-C-06 | 성능(통신 오버헤드)을 위한 zero-copy 보안 채널 구조 설계 | FR-04, QA-01, QA-02, QA-04, QA-05, QA-06 | DP-C-05 | DP4 |
| DP-C-07 | 성능(실시간 처리) 및 자원 효율을 위한 HW IP 중재 구조 설계 | FR-03, QA-03, QA-06, QA-11, CONST-05 | DP-C-05 | DP5 |
...
flowchart TD
    ...
    class DPC04,DPC05,DPC06,DPC07,DPC03 critical
...
#### DP1: 기밀성, 확장성, 가용성을 위한 pVM 관리 골격 설계 (→ DP-C-04)
...
#### DP2: 무결성 보장을 위한 보안 워크로드 실행 방식 설계 (→ DP-C-05)
...
#### DP3: GP API Backward Compatibility를 위한 TrustZone 연동 구조 설계 (→ DP-C-03)
...
**선행 DP Candidate**
- DP-C-05 (보안 워크로드 실행 방식): 채널의 양단은 실행되는 워크로드(Secure Camera·Secure AI)이므로, 워크로드 실행·격리 구조가 확정되어야 채널의 종단점과 권한 가정을 정할 수 있다. HW IP 중재(DP-C-07)의 DMA 버퍼 소유권 결정과는 상호 참조하여 정합을 맞춘다.
...
#### DP7: 시스템의 보안 정책 신뢰를 위한 TCB 바운더리 결정 (→ DP-C-01)
