# DP-02 결정 과정: 보안 정책·권한 집행 모델

> 일자: 2026-06-11
> 관련 문서: `05_design_points.md` DP-02, `06_dp01_trust_model.md`
...
- DP-08의 패키지 manifest 권한과 채널 상대 인증의 pVM 아이덴티티를 어떻게 연결할 것인가?
...
- **철회**: pVM 종료·워크로드 삭제 시 생명주기 관리(DP-04)가 자원 회수, TEE가 키 방출 중단
...
- **단점**: 정책 pVM이 모든 권한 결정의 단일 신뢰 지점이 되어 TCB 편입이 필수(부트스트랩·자원 비용·DP-01 조건부 규칙). 모든 권한 판단 경로에 pVM 왕복이 끼어들어 지연이 늘고(QA-11), DP-04~05보다 먼저 완성되어야 하는 순서 부담이 크다.
...
| E4. 집행 일관성 | DP-04~05가 같은 권한 가정 위에서 설계될 수 있는가 |
...
| E4 집행 일관성 | 상 — 단일 manifest가 DP-04~05의 공통 기준 | 중 — 모든 권한 경로의 정책 pVM 경유 강제 필요 | 하 — 우회 시 사후 감사만 남음 |
...
3. SMMU 설정 경로가 Host 드라이버에 남는 경우 DP-05과 충돌 가능 — HW IP 권한 집행 지점은 별도 검증 필요.
...
**동의**: 후보 A 채택(신뢰 모델 6.5절 DP-02 전제와 완전 정합), 정의/검증/집행 분리, fail-closed 집행, 후보 C 부적합 판정, 3.3절 결함 지적 전부 타당.

**이견 1 (CRITICAL)**: 후보 A의 핵심 전제 — "EL2 기 제공 메커니즘이 capability 게이트를 지원하는가" — 가 미확인이며, 거짓이면 후보 A의 E1 "상" 자체가 무너진다. EL2 hypercall이 Host가 요청한 메모리 share를 manifest 권한 대조 없이 그대로 수행한다면 집행 지점이 실제로는 비신뢰 Host 드라이버에 남아 D-1 위반이 된다. **능력 부여 길목별로 실제 게이트가 어느 TCB 요소에서 일어나는지 1:1 매핑이 검증되기 전까지 후보 A는 "조건부"다.** 이 전제가 거짓이면 DP-02 전면 재설계가 필요하다.

**이견 2 (MAJOR)**: SMMU 설정 경로가 Host 드라이버에 남는 경우 비신뢰 주체가 DMA 격리 경계를 통제하게 되어 DP-05 전제와 충돌한다. DP-02 단독으로 닫을 수 없으므로 DP-05에 사활적 입력으로 명시적 핸드오프해야 한다.
...
| G2 | **manifest 보안 속성 정의**: 스키마·버전·서명 키 체계·폐기/회전·롤백 방지·측정값↔권한 바인딩을 정의한다. 스키마·패키징은 DP-08, 측정값 바인딩은 DP-09, 아이덴티티 연결은 채널 상대 인증과 분담 | DP-08, DP-09, 채널 상대 인증 |
| G3 | **SMMU 집행 주체 핸드오프**: SMMU 매핑 권한의 집행 주체 확정을 DP-05의 사활적 입력으로 전달한다. SMMU 설정이 Host 드라이버에 남는 구조는 본 정책 모델과 충돌함을 명시 | DP-05 |
| G4 | **철회 모델 확장**: pVM 종료·키 방출 중단 외에 런타임 채널 강제 해제(DP-06), HW IP 할당 회수(DP-05)의 철회 경로를 해당 DP 설계에 요구 사항으로 전달 | DP-05, DP-06 |
| G5 | **measured 권한 바인딩**: manifest 권한은 DP-09 부팅 측정값에 바인딩되어야 "승인된 워크로드"와 "Host가 만든 임의 인스턴스"를 구분할 수 있다 (DP-03 G2와 동일 근원). **[역반영 1단계 완료 — 2026-06-12, 14 문서 5.3절]** 바인딩 참조 필드 = DP-09 측정값 포맷의 manifest hash·manifest version(anti-rollback)으로 확정. **완전 폐쇄는 H7 HV 확인 후**(측정 주체·시점 실증) | DP-09 G3, H7 |
...
| SMMU 매핑 | DP-05에서 확정 (G3) | Host 단독 설정 구조는 불허 |
...
- DP-04~05은 "권한 = 서명된 manifest의 정적 선언, 집행 = TCB 게이트 fail-closed"를 공통 전제로 설계한다.
- Host 프레임워크는 정책의 운반자·요청자이며 어떤 길목에서도 단독 결정자가 아니다.
- 키 방출 게이트(TEE)는 모든 워크로드 보호 자산의 최종 방어선이다 — DP-07 설계의 출발 전제.
