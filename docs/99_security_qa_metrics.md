...
보안 요구사항은 "노출되지 않는다"와 같이 부정형(negative)으로 기술되어 직접 측정이 어렵다. 그러나 하이퍼바이저·TEE 격리 분야에서는 아래와 같이 정량화된 검증 체계가 확립되어 있으며, 특히 본 과제와 동일한 기술 기반인 pKVM이 이미 이 체계로 인증을 획득한 선례가 있다.
