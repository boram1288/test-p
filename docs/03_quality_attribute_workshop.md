...
> 관련 UC: UC-04 (도메인 간 보안 데이터 전송), UC-06 (Secure OS ENC/DEC 명령 전송)
...
### QS-02: 성능 — 실시간 처리 (QA-02)

> 관련 UC: UC-03 (Camera/AI HW 공유 사용), UC-04 (도메인 간 보안 데이터 전송)
...
### QS-03: 성능 — 도메인 간 통신 오버헤드 (QA-03)

> 관련 UC: UC-04 (도메인 간 보안 데이터 전송)
...
### QS-05: 확장성 — 신규 Workload 수용 (QA-05)
...
### QS-06: 변경 용이성 — Secure OS 교체 (QA-06)
...
| QS-02 | 성능 (실시간 처리) | **H** | 실시간성 미달 시 제품 기능 자체가 불성립(VOS-02). 보안을 위해 성능을 포기할 수 없음 | **H** | 가상화 경계를 넘는 데이터 경로에서 비격리 대비 10% 이내 저하 달성이 어려움 |
| QS-03 | 성능 (통신 오버헤드) | **H** | QS-02의 하위 조건. 도메인 간 전달이 병목이면 파이프라인 실시간성 불성립 | M | zero-copy 공유 메모리 설계로 달성 가능하나 격리 보장과의 양립 설계 필요 |
| QS-04 | 자원 효율 | M | 추가 메모리/전력 소모는 제품 **가격 경쟁력**(VOS-04)에 영향을 주는 요소이나, 제품 출시 전제나 사고 피해처럼 미충족 시 과제가 무산되는 수준은 아니며 설계 최적화로 관리 가능 | M | pVM 메모리 예약/동적 회수 설계로 대응 가능 |
| QS-05 | 확장성 (Workload 수용) | **H** | 과제 핵심 키워드(VOS-05). 단일 시나리오 솔루션은 사업 경쟁력 없음. AVF 대비 차별화 지점 | **H** | 플러그인형 패키징/로딩 구조, 인터페이스 안정성 등 Framework 전체 구조를 결정 |
| QS-06 | 변경 용이성 (Secure OS 교체) | M | 서드파티 Secure OS 수용(VOS-15) 항목이나, 표준 GP(GlobalPlatform) TEE API 지원만으로 교체가 가능하여 별도 구조 차별화가 불필요. 사업 영향은 중간 | M | QS-05의 확장 구조가 갖춰지면 인터페이스 추상화로 달성 가능 |
| QS-07 | 가용성 (장애 격리) | M | 로봇 기본 동작 보호는 중요하나, 격리 구조 자체가 장애 전파를 1차 차단 | M | 생명주기 관리의 자원 회수/재시작 설계로 대응 |
| QS-08 | 시험 용이성 | M | 검증 불가능한 보안 주장은 수용 불가(VOS-13)/규제 증빙(VOS-17)과 연계 | M | 침해 모사 도구/시험 훅 설계 필요하나 구조 전체를 결정하지는 않음 |
| QS-09 | 성능 (HW IP 공유 오버헤드) | **H** | Host 일반 기능과 보안 기능의 동시 성립 조건(VOS-02). 미충족 시 제품 기능 절충 발생 | **H** | 다중 Context 미지원 HW의 SW 중재는 보안(QS-03)/성능(QS-02)과 삼중 상충. 본 과제 고유의 난제 |
...
|:----:|---------------|--------------|------|
| 1 | **보안 (Security)** | QS-01 (Host 침해 기밀성) | |
| 1 | **성능 (Performance)** | QS-02 (실시간 처리), QS-09 (HW IP 공유 오버헤드) | QS-03(통신 오버헤드)는 2순위로 그룹에 포함 |
| 1 | **확장성 (Extensibility)** | QS-05 (Workload 수용) | 종속 2순위 시나리오 없음 (QS-06은 중요도 재평가로 3순위로 이동) |
| 3 | 자원 효율, 변경 용이성, 가용성, 시험 용이성 | QS-04, QS-06, QS-07, QS-08 | 구조 결정 후 설계/검증 단계에서 충족 확인. 자원 효율은 독립 교차 관심사로 메모리/전력 footprint 제약으로 작용 |

### 4.3 핵심 QA 간 상충 관계

선정된 세 핵심 QA(1순위)는 서로 상충하며, 여기에 가격 경쟁력과 연계된 자원 효율(3순위)이 교차 제약으로 작용한다. 이 상충의 해소가 아키텍처 설계의 중심 과제다.

```mermaid
flowchart LR
    SEC["보안<br/>QS-01"]
    PERF["성능<br/>QS-02, QS-09 (+QS-03)"]
    EXT["확장성<br/>QS-05"]
    RES["자원 효율<br/>QS-04"]
...
- **보안 vs 성능**: 격리 경계(Stage-2, SMMU, 보안 채널)를 강화할수록 데이터 경로 비용이 증가한다. zero-copy 공유와 격리 보장의 양립(QS-03), HW 가속과 격리의 양립(QS-02/11)이 핵심 상충 지점이다.
- **보안 vs 확장성**: Workload 동적 탑재(QS-05)는 검증되지 않은 코드의 진입 경로가 될 수 있어, 패키지 검증/서명 구조가 전제되어야 한다.
- **성능 vs 확장성**: 다양한 Workload를 수용하기 위한 추상화 계층은 호출 경로를 늘려 오버헤드를 유발할 수 있다.
- **자원 효율(3순위) 상충**: 격리 경계(보안)와 추상화 계층(확장성)은 모두 추가 메모리/전력을 소모하므로 자원 효율과 상충한다. 가격 경쟁력 제약(중요도 M) 하에서 격리/확장 구조의 메모리/전력 footprint를 탑재 한도 이내로 최소화하는 설계가 요구된다.

---

## 다음 단계

선정된 핵심 품질 속성(보안, 성능, 확장성)과 그 시나리오를, 가격 경쟁력과 연계된 자원 효율(3순위) 제약과 함께 FR/CONST와 결합하여, **Architectural Driver 선정** 단계에서 아키텍처 설계를 주도할 동인을 확정한다.
