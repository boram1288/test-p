...
| 요소 | 내용 |
|------|------|
| 자극원 | 침해된 보안 워크로드(예: 취약점이 악용된 Secure Camera pVM) |
...
| 자극 | (a) pVM이 사용 중인 ISP·NPU의 DMA를 조작하여 격리 메모리 접근 시도, (b) HW IP 사용 주체 전환 직후 이전 도메인의 잔류 데이터 읽기 시도 |
| 환경 | HW IP가 SW 중재로 Host·pVM 간 공유되는 상태 |
| 대상 | 격리 메모리, HW IP 내부 버퍼·레지스터의 잔류 데이터 |
| 응답 | SMMU/IOMMU에 의해 DMA 접근이 차단되고, 전환 시 잔류 데이터가 소거된다 |
...
| 대상 | End-to-End 파이프라인 (캡처→ISP→NPU 추론→판단) |
| 응답 | ISP·NPU HW 가속으로 프레임 드롭 없이 처리 |
...
|------|------|
| 자극원 | 시스템 통합자(로봇 제조사) |
| 자극 | 보안 프레임워크 + pVM 2개(Camera, AI) 상시 운용 |
| 환경 | 로봇 제품의 통상 동작 상태 |
| 대상 | SoC 메모리·전력 예산 |
| 응답 | 비격리 구성 대비 추가 자원 소모가 제품 탑재 가능 한도 이내 |
| 응답 측정치 | 프레임워크·격리 운용에 따른 추가 메모리 256MB 이하, 전력 증가 5% 이내 (가정치) |

### QS-07: 확장성 — 신규 워크로드 수용 (QA-07)

| 요소 | 내용 |
|------|------|
| 자극원 | 로봇 제조사 / 워크로드 개발자 |
| 자극 | 신규 보안 워크로드(예: 개인정보 처리, 펌웨어 보호) 추가 요구 |
| 환경 | 프레임워크가 제품에 배포·운용 중인 상태 |
| 대상 | 프레임워크 본체(미들웨어·커널 드라이버) |
| 응답 | 펌웨어 재배포·프레임워크 수정 없이 워크로드 패키징·탑재만으로 수용된다 |
| 응답 측정치 | 프레임워크 코어 수정 0 LoC, 워크로드 패키지 작성·탑재만으로 동작 |
...
| 대상 | Secure OS와 무관한 SW(프레임워크, Host 미들웨어, 타 워크로드) |
...
| 응답 | 장애가 해당 pVM에 한정되고, 프레임워크가 자원을 안전하게 회수한 뒤 재시작한다 |
...
| 자극 | 단일 컨텍스트 HW IP(ISP·NPU)에 대한 동시 사용 요구 |
...
| QS-03 | 보안 (DMA 경로 격리) | **H** | HW 가속(R-2)과 격리(R-1)를 동시에 성립시키는 조건. 미충족 시 격리 자체가 무효 | **H** | 단일 컨텍스트 HW의 공유·전환·소거를 SMMU 제어와 결합해야 함. HW IP별 특성 의존 |
...
| QS-06 | 자원 효율 | M | 제품 탑재 조건이나, 임베디드 SoC 사양 향상 추세로 협상 여지 있음 | M | pVM 메모리 예약·동적 회수 설계로 대응 가능 |
| QS-07 | 확장성 (워크로드 수용) | **H** | 과제 핵심 키워드(VOS-05). 단일 시나리오 솔루션은 사업 경쟁력 없음. AVF 대비 차별화 지점 | **H** | 플러그인형 패키징·로딩 구조, 인터페이스 안정성 등 프레임워크 전체 구조를 결정 |
| QS-08 | 변경 용이성 (Secure OS 교체) | **H** | 서드파티 Secure OS 수용(VOS-15)은 확장성의 핵심 검증 항목 | M | QS-07의 확장 구조가 갖춰지면 인터페이스 추상화로 달성 가능 |
| QS-09 | 가용성 (장애 격리) | M | 로봇 기본 동작 보호는 중요하나, 격리 구조 자체가 장애 전파를 1차 차단 | M | 생명주기 관리(FR-09)의 자원 회수·재시작 설계로 대응 |
| QS-10 | 시험 용이성 | M | 검증 불가능한 보안 주장은 수용 불가(VOS-13)·규제 증빙(VOS-17)과 연계 | M | 침해 모사 도구·시험 훅 설계 필요하나 구조 전체를 결정하지는 않음 |
| QS-11 | 성능 (HW IP 공유 오버헤드) | **H** | Host 일반 기능과 보안 기능의 동시 성립 조건(VOS-02). 미충족 시 제품 기능 절충 발생 | **H** | 다중 컨텍스트 미지원 HW의 SW 중재는 보안(QS-03)·성능(QS-04)과 삼중 상충. 본 과제 고유의 난제 |
...
- **2순위 (핵심 QA 그룹에 종속)**: 중요도 H이고 난이도 M — 1순위 해결 구조 위에서 함께 달성되도록 설계
...
| 1 | **확장성 (Extensibility)** | QS-07 (워크로드 수용) | QS-08(Secure OS 교체)은 2순위로 그룹에 포함 |
| 3 | 자원 효율, 가용성, 시험 용이성 | QS-06, QS-09, QS-10 | 구조 결정 후 설계·검증 단계에서 충족 확인 |

### 4.3 핵심 QA 간 상충 관계

선정된 세 핵심 QA는 서로 상충하며, 이 상충의 해소가 아키텍처 설계의 중심 과제다.

```mermaid
flowchart LR
    SEC["보안<br/>QS-01, QS-03 (+QS-02)"]
    PERF["성능<br/>QS-04, QS-11 (+QS-05)"]
    EXT["확장성<br/>QS-07 (+QS-08)"]
...
- **보안 vs 확장성**: 워크로드 동적 탑재(QS-07)는 검증되지 않은 코드의 진입 경로가 될 수 있어, 패키지 검증·서명 구조가 전제되어야 한다.
- **성능 vs 확장성**: 다양한 워크로드를 수용하기 위한 추상화 계층은 호출 경로를 늘려 오버헤드를 유발할 수 있다.

---

## 다음 단계

선정된 핵심 품질 속성(보안, 성능, 확장성)과 그 시나리오를 FR·CONST와 결합하여, **Architectural Driver 선정** 단계에서 아키텍처 설계를 주도할 동인을 확정한다.
