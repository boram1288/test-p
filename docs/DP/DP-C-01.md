...
| 무결성 | pVM 이미지·워크로드 변조 방지, pVM 메모리, 권한 정책 | AT-1, AT-2, AT-3에 대해 보호 대상의 변조와 위조 실행을 방지한다. AT-3은 타 pVM 자산에 한정한다. |
...
4. **TrustZone TEE**는 필요한 경우 pVM 이미지 측정값, 키, 정책 확인을 수행한다.
...
3. **TrustZone TEE**는 필요한 경우 워크로드 측정값, 서명, 정책, 키 방출 조건을 확인한다.
4. **pVM**은 검증된 워크로드를 자기 격리 메모리 안에서 로딩한다.
