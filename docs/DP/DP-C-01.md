...
| ID | 결정 항목 |
|----|----------|
| D-1 | pVM 측 프레임워크(미들웨어/커널 드라이버)의 TCB 포함 여부 |
...
| ID | 공격자 능력 | 결정 | 근거 |
|----|------------|:----:|------|
| AT-1 | **Host 사용자 공간 권한 침해** — 미들웨어 및 앱 권한 탈취, 프레임워크 API 오용 | **포함** | AT-2에 포섭되는 하위 수준이나, 프레임워크 API가 1차 공격 표면이므로 별도 명시 |
| AT-2 | **Host 커널 권한 침해** — 악성 커널 모듈, KVM 인터페이스 오용, 페이지 테이블 조작, DMA 조작 시도 | **포함** | QS-01의 자극원 그 자체. R-1(Host 비신뢰)이 과제의 존재 이유이므로 협상 불가 |
| AT-3 | **pVM 내부 권한 침해** — 취약점이 악용된 보안 워크로드(예: 침해된 Secure Camera)가 타 pVM/Host/HW IP를 공격 | **포함** | QS-02의 자극원. 다중 도메인 구조(R-3)에서 도메인 상호 비신뢰는 QA-02의 전제 |
...
| 보호 속성 | 보호 대상 | 보장 범위 |
|----------|----------|----------|
| 기밀성 | pVM 이미지/워크로드 평문, pVM 내 영상/모델/추론 데이터 | AT-1, AT-2, AT-3에 대해 보호 대상의 평문 노출을 방지한다. AT-3은 타 pVM 자산에 한정한다. |
| 무결성 | 승인된 pVM 이미지/워크로드 기준값, pVM 메모리, 권한 정책 | AT-1, AT-2, AT-3에 대해 보호 대상의 변조와 위조 실행을 방지한다. AT-3은 타 pVM 자산에 한정한다. |
...
| 후보 | 구성 | 설명 |
|------|------|------|
| **A. 최소 TCB안** | TCB = {HW 메커니즘, 부트체인, EL3, S-EL1(기존 TEE), EL2(pKVM), 승인된 pVM 이미지/워크로드 기준값} | Host 측 프레임워크 전부 비신뢰. 보안 보장은 Stage-2/SMMU 메커니즘과 승인 기준값에 의존. 신규 신뢰 컴포넌트 추가 없음 |
| **B. 서비스 pVM 확장안** | A + 검증자/로거/키 중개 등을 담는 전용 서비스 pVM | 신뢰가 필요한 기능을 Linux 네이티브 게스트로 구현한 서비스 pVM에 배치한다. 단, 해당 서비스 pVM의 이미지/워크로드가 승인 기준값으로 검증된 뒤에만 TCB에 편입된다 |
...
- **B 단독은 부트스트랩 순환에 빠진다.** 서비스 pVM을 신뢰하려면 그 이미지/워크로드가 승인 기준값과 일치하는지 누군가 검증해야 하는데, 검증 주체가 다시 필요해진다.
...
3. **서비스 pVM (B 요소) — 조건부 2차 앵커**: TEE 자원 제약 또는 무회귀 위험이 큰 기능(예: 고빈도 로그 수집, HW IP 중재)에 한해 도입한다. 단 두 가지 편입 조건을 만족해야 한다: (i) 해당 pVM 이미지/워크로드의 검증이 TEE 앵커(또는 Secure Boot 체인)에 연결될 것, (ii) TCB 편입을 본 문서의 개정으로 기록하고 영향 평가를 거칠 것.
...
| SW 기준값 | 승인된 pVM 이미지/워크로드 기준값 | 서명/측정값/manifest로 식별 | 실행 대상의 원본성/정책 바인딩 기준 |
| (조건부) | 승인된 서비스 pVM 이미지/워크로드 | TEE 또는 Secure Boot 체인에 연결된 검증 + 본 문서 개정 | 도입 시점에 TCB 편입을 명시적으로 기록 |

**비신뢰(TCB 밖)**: Host EL1/EL0 전부 — Linux 커널, 본 과제가 개발하는 커널 드라이버/미들웨어/프레임워크 포함. pVM들은 서로에 대해 비신뢰. 단, TCB 기능을 수행하도록 승인된 서비스 pVM은 이미지/워크로드 검증과 본 문서 개정을 거친 뒤 TCB에 편입된다.
...
1. **Host 프레임워크**가 워크로드 실행 요청을 전달한다.
2. **Host 커널 / VMM**은 워크로드 패키지를 pVM에 전달하지만, 보안 판단은 수행하지 않는다.
3. **TrustZone TEE**는 필요한 경우 워크로드 측정값, 서명, manifest, 정책, 키 방출 조건을 승인 기준값과 대조한다.
4. **pVM**은 승인 기준을 통과한 워크로드만 자기 격리 메모리 안에서 로딩한다.
5. **EL2 pKVM**은 로딩 이후에도 Host가 pVM 메모리를 읽거나 쓰지 못하도록 Stage-2 접근을 차단한다.
6. **pVM**은 영상, 모델, 추론 데이터 등 보호 자산을 자기 격리 영역 안에서 처리한다.

**pVM 실행 중 보호**

1. **pVM**은 보안 워크로드를 계속 실행한다.
2. **EL2 pKVM**은 Host의 pVM 메모리 읽기/쓰기 접근을 차단한다.
3. **SMMU / IOMMU**는 DMA 경로에서 Host나 장치가 pVM 보호 메모리에 직접 접근하지 못하게 한다.
4. **Host 프레임워크**는 생명주기 관리, 상태 모니터링, 재시작 같은 관리 기능을 수행하지만 보안 보장 주체는 아니다.
...
3. **Host 프레임워크**는 FR-09에 따라 장애 pVM의 자원을 회수하고 재시작을 시도한다.
...
| 무결성 (이미지/워크로드) | 승인된 이미지/워크로드 기준값 + 서명/측정 검증 | DP-C-05, DP-C-02 |
...
flowchart TB
    ...
    subgraph tcb["TCB"]
        ...
        BASE["승인된 pVM 이미지/워크로드 기준값<br/>(서명/측정값/manifest)"]
        HW["HW: Stage-2 MMU/SMMU<br/>부트체인"]
        SVC["(조건부) 승인된 서비스 pVM 이미지/워크로드<br/>편입 시 본 문서 개정"]
    end
...
| DP-C-05/C-08 (저장/부팅) | 신뢰 앵커 선택 시 5.4절의 우선순위 규칙(EL2 메커니즘 → TEE → 조건부 서비스 pVM)을 적용한다. pVM 이미지/워크로드는 승인 기준값으로 식별하고, Host는 전달자 역할에 한정한다 |
...
| D-4 | 신뢰 주체 | 기본 TCB = {HW, 부트체인, EL3, 기존 TEE, EL2, 승인된 pVM 이미지/워크로드 기준값}. 신뢰 앵커 우선순위: EL2 기 제공 메커니즘 → TEE(1차) → 조건부 서비스 pVM(2차) |
