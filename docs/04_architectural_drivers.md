...
| QA-02 | 성능 — 실시간 처리 (QS-02) | 2순위 |
| QA-03 | 확장성 — Workload 수용 (QS-03) | 3순위 |
| QA-04 | 성능 — 통신 오버헤드 (QS-04) | 4순위 |
| QA-05 | 가용성 — pVM 장애 격리 (QS-05) | 5순위 |
| QA-06 | 자원 효율 (QS-06) | 6순위 |
| QA-07 | 시험 용이성 (QS-07) | 7순위 |
| QA-08 | 변경 용이성 — Secure OS 교체 (QS-08) | 8순위 |
...
| QA-02 | 성능 — 실시간 처리 (QS-02) | 2순위 | **선정** | 격리 경계 통과 최소화, 데이터/제어 경로 분리 등 런타임 구조를 결정 |
| QA-03 | 확장성 — Workload 수용 (QS-03) | 3순위 | **선정** | Workload 패키징/검증/탑재 구조와 Framework 확장 경계를 결정 |
| QA-04 | 성능 — 통신 오버헤드 (QS-04) | 4순위 | **선정** | zero-copy 공유 메모리 채널과 격리 보장의 양립 구조를 결정 |
| QA-05 | 가용성 — pVM 장애 격리 (QS-05) | 5순위 | **선정** | pVM 생명주기 관리 설계 안에서 충족 여부를 확인 |
| QA-06 | 자원 효율 (QS-06) | 6순위 | 제외 | 메모리/전력 footprint 제약으로 작용하되 독자적 구조 결정성은 낮음 |
| QA-07 | 시험 용이성 (QS-07) | 7순위 | 제외 | 침해 모사 도구/시험 훅은 구조 결정 후 검증 설계에서 대응 |
| QA-08 | 변경 용이성 — Secure OS 교체 (QS-08) | 8순위 | 제외 | 표준 인터페이스와 어댑터 경계로 대응하며 초기 핵심 driver에서는 제외 |
...
| 품질 (QA) | QA-01, QA-02, QA-03, QA-04, QA-05 | QA-06, QA-07, QA-08 |
