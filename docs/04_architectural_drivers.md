...
|----|---------|---------|:----:|----------|
| FR-01 | pVM 생성/시작/정지/종료 | UC-01 | **선정** | 프레임워크 본체(미들웨어/드라이버)의 책임 분할과 컴포넌트 경계를 결정 |
| FR-02 | 다중 pVM 동시 운용 | UC-02 | **선정** | 다중 격리 도메인 구조(도메인 단위 독립 관리)를 강제. TrustZone 이진 구조와의 차별화 지점 |
| FR-03 | Camera/AI HW 공유 사용 | UC-03 | **선정** | HW IP 중재 계층이라는 신규 구조 요소를 강제. 중재자의 위치(Host 커널/전용 pVM)가 전체 구조를 좌우 |
| FR-04 | 격리 도메인 간 보안 데이터 전송 | UC-04 | **선정** | 도메인 간 데이터 경로(공유 메모리/RPC 채널) 구조와 격리 영역 내 데이터 유지 규칙을 결정 |
| FR-05 | 보안 워크로드 동적 탑재 | UC-05 | **선정** | 플러그인형 패키징/로딩 구조와 패키지 검증 체계를 강제 |
| FR-06 | Secure OS ENC/DEC 명령 전송 | UC-06 | **선정** | 프레임워크-게스트 간 ABI(부팅 규약, 가상 디바이스, 통신 규약) 고정 및 pVM→TEE 요청 라우팅(SMC 포워딩)과 기존 경로 무회귀를 만족하는 연동 계층을 결정 |
| FR-07 | pVM 모니터링 및 장애 복구 | UC-07 | 제외 | FR-01의 생명주기 관리 구조 안에서 구현 가능. 독자적 구조 결정 없음 |
...
| QA-08 | 변경 용이성 — Secure OS 교체 (QS-08) | 3순위 | 제외 | 표준 GP(GlobalPlatform) TEE API 지원만으로 교체 가능. QA-07 확장 구조/FR-07(Secure OS 실행) 위에서 충족되며 독자적 구조 결정 없음 |
...
| 구분 | 선정 Driver | 제외 |
|------|------------|------|
| 기능 (FR) | FR-01, FR-02, FR-03, FR-04, FR-05, FR-06 | FR-07 |
...
| 5 | 보안 워크로드 실행 방식 결정 | FR-05, QA-07, CONST-06 | T-4 |
...
| 1-b. 유즈케이스 도출 | `01_use_case.md`, `01_use_case_spec.md` | UC-01~UC-07 (액터: 사용자, FR-01~FR-09 대응) |
| 2. 요구사항 도출 | `02_requirements.md` | FR 7건, QA 11건, CONST 6건 (VOS 추적성 확보, UC 1:1 대응) |
