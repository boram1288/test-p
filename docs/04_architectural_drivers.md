...
|----|---------|---------|:----:|----------|
| FR-01 | pVM 생명주기 관리 | UC-01 | **선정** | 프레임워크 본체(미들웨어·드라이버)의 책임 분할과 컴포넌트 경계를 결정 |
| FR-02 | 다중 pVM 동시 운용 | UC-02 | **선정** | 다중 격리 도메인 구조(도메인 단위 독립 관리)를 강제. TrustZone 이진 구조와의 차별화 지점 |
| FR-03 | HW IP 보안 공유(할당·중재) | UC-03 | **선정** | HW IP 중재 계층이라는 신규 구조 요소를 강제. 중재자의 위치(Host 커널/전용 pVM)가 전체 구조를 좌우 |
| FR-04 | 격리 도메인 간 보안 통신 | UC-04 | **선정** | 도메인 간 데이터 경로(공유 메모리·RPC 채널) 구조를 결정 |
| FR-05 | 데이터 격리 영역 내 유지 | UC-04 | 제외 | 데이터 흐름 규칙으로, QA-01(기밀성)을 충족하는 격리 구조 위에서 달성됨. 독자적 구조 결정 없음 |
| FR-06 | 보안 워크로드 동적 탑재 | UC-05 | **선정** | 플러그인형 패키징·로딩 구조와 패키지 검증 체계를 강제 |
| FR-07 | Secure OS pVM 실행 지원 | UC-06 | **선정** | 프레임워크-게스트 간 ABI(부팅 규약, 가상 디바이스, 통신 규약)의 초기 고정을 강제 |
| FR-08 | TrustZone 공존·연동 | UC-06 | **선정** | pVM→TEE 요청 라우팅(SMC 포워딩)과 기존 경로 무회귀를 만족하는 세계 간 연동 계층을 결정 |
| FR-09 | pVM 상태 모니터링·장애 처리 | UC-07 | 제외 | FR-01의 생명주기 관리 구조 안에서 구현 가능. 독자적 구조 결정 없음 |
...
| 5 | 보안 워크로드 실행 방식 결정 | FR-06, FR-07, QA-07, CONST-06 | T-4 |
