...
| QA-02 | 성능 — 실시간 처리 (QS-02) | 1순위 | **선정** | 격리 경계 통과 최소화, 데이터/제어 경로 분리 등 데이터 경로 구조를 결정 |
| QA-03 | 성능 — 통신 오버헤드 (QS-03) | 2순위 | **선정** | zero-copy 공유 메모리 채널과 격리 보장의 양립 구조를 결정 |
| QA-04 | 자원 효율 (QS-04) | 3순위 | 제외 | 가격 경쟁력과 연계된 자원 제약(VOS-04)이나 독자적 구조를 결정하지는 않음. 격리 경계/추상화 계층의 메모리/전력 footprint를 탑재 한도 이내로 유지하는 교차 제약으로, 구조 결정 후 설계/검증 단계에서 충족 확인 |
| QA-05 | 확장성 — Workload 수용 (QS-05) | 1순위 | **선정** | 과제 핵심 키워드. 플러그인 구조와 인터페이스 계약의 안정성을 강제 |
| QA-06 | 변경 용이성 — Secure OS 교체 (QS-06) | 3순위 | 제외 | 표준 GP(GlobalPlatform) TEE API 지원만으로 교체 가능. QA-05 확장 구조/FR-06(Secure OS 실행) 위에서 충족되며 독자적 구조 결정 없음 |
| QA-07 | 가용성 (QS-07) | 3순위 | 제외 | 격리 구조가 장애 전파를 1차 차단. 생명주기 관리(FR-01) 설계 안에서 대응 |
| QA-08 | 시험 용이성 (QS-08) | 3순위 | 제외 | 침해 모사 도구/시험 훅은 구조 결정 후 검증 설계에서 대응 |
| QA-09 | 성능 — HW IP 공유 오버헤드 (QS-09) | 1순위 | **선정** | 단일 Context HW의 SW 중재에서 보안/성능이 동시에 걸리는 본 과제 고유의 난제 |

**선정 결과: QA-01, QA-02, QA-03, QA-05, QA-09 (5건)**
...
| CONST-05 | 타겟 HW (단일 Context HW IP, HW 변경 불가) | HW IP 공유를 HW 가상화 없이 SW 중재로만 실현해야 함. FR-03/QA-09 해법의 선택지를 한정 |
...
선정된 driver는 총 **17건** (FR 6 + QA 5 + CONST 6)이다.

| 구분 | 선정 Driver | 제외 |
|------|------------|------|
| 기능 (FR) | FR-01, FR-02, FR-03, FR-04, FR-05, FR-06 | - |
| 품질 (QA) | QA-01, QA-02, QA-03, QA-05, QA-09 | QA-04, QA-06, QA-07, QA-08 |
...
flowchart TB
    ...
    subgraph qa_d["품질 Driver (구조 주도)"]
        SEC["보안<br/>QA-01"]
        PERF["성능<br/>QA-02, 03, 09"]
        EXT["확장성<br/>QA-05"]
        RES["자원 효율<br/>QA-04 (3순위 교차 제약)"]
    end
...
|:----:|------|------------|----------|
| 1 | 신뢰 모델 확정 — TCB 범위(Host 측 Framework의 TCB 제외 여부), 공격자 모델, 보호 범위(기밀성/무결성 보장 / 가용성 제외 등) 결정 | QA-01, CONST-02 | T-1~T-5 공통 전제 (특히 T-1의 책임 배치 결정) |
| 2 | 생명주기/다중 pVM 관리 골격 설계 | FR-01, FR-02, CONST-01 (QA-04 footprint 확인) | T-1 |
| 3 | HW IP 중재 구조 실현 가능성 검증 (PoC) | FR-03, QA-09, CONST-05 | T-2 |
| 4 | 통신/데이터 경로(zero-copy 보안 채널) 구조 설계 | FR-04, QA-02, QA-03 | T-3 |
| 5 | 보안 Workload 실행 방식 결정 | FR-05, QA-05, CONST-06 | T-4 |
...
| 2. 요구사항 도출 | `02_requirements.md` | FR 6건, QA 9건, CONST 6건 (VOS 추적성 확보) |
| 3. 품질 속성 선정 | `03_quality_attribute_workshop.md` | 시나리오 QS-01~09, 핵심 QA: 보안/성능/확장성 |
| 4. Architectural Driver 선정 | `04_architectural_drivers.md` (본 문서) | Driver 17건 = 중요 FR 6 + 핵심 QA 5 + CONST 전체 6 |
