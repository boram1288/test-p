# DP-08 결정 과정: 보안 워크로드·Secure OS 탑재 방식 결정

> 일자: 2026-06-12
> 관련 문서: `05_design_points.md` DP-08, `07_dp02_policy_model.md`, `11_dp06_zero_copy_channel.md`, `12_dp07_at_rest_protection.md`, `09_dp04_lifecycle_skeleton.md`
...
- **DP-02**: 패키지에는 서명된 manifest가 포함되며, manifest 권한은 TCB 게이트(TEE·EL2 집행 지점)에서 fail-closed로 대조된다. manifest 속성 정의는 DP-08(패키지 형식)·DP-09 측정값이 분담한다(DP-02 G2).
...
5. **(정합) 패키지 구획 ↔ 측정 단위 1:1** — DP-08 패키지 구획 경계와 DP-09 측정 단위가 동일 구획 분류표를 참조해야 한다. 측정 종료점에 보호 자산 chunk manifest 포함 여부가 미고정이었다.
