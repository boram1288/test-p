...
### 2.3 확장성 (Scalability) 정의 — 2023년 신설 부특성

ISO/IEC 25010:2023에서 유연성(Flexibility) 특성의 부특성으로 신설되었다.

- **정의**: 제품이 증가하거나 감소하는 작업 부하(workload)를 처리할 수 있는 정도, 또는 그러한 변동성을 처리하기 위해 자신의 용량(capacity)을 조정할 수 있는 정도.
- **해설**: 사용자 수, 트랜잭션 양, 데이터 규모 등의 변화에 대응하여 시스템 용량을 늘리거나(scale-up/scale-out) 줄일(scale-down/scale-in) 수 있는 능력을 의미한다. 성능 효율성의 부특성인 용량(Capacity)이 "현재 시점의 최대 한계가 요구사항을 충족하는 정도"를 다루는 반면, 확장성은 "요구 변화에 맞추어 그 한계 자체를 조정할 수 있는 능력"을 다룬다는 점에서 구별된다.
- **측정 관련**: ISO/IEC 25023:2016은 25010:2011 기준이므로 확장성에 대한 표준 측정 지표를 제공하지 않는다. 측정이 필요한 경우 ISO/IEC 25021의 QME 정의 절차에 따라 자체 지표(예: 부하 N배 증가 시 처리율 유지 비율, 용량 증설 소요 시간 등)를 정의해야 한다(4절 5항 참조).
