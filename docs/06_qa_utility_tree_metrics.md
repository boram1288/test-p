...
| HW 접근 격리 (S2MPU) | SEC-02 | Camera/AI HW 사용 주체 전환 시 S2MPU 권한 중첩 구간과 잔류 데이터 노출이 없다 | **게이트**: 권한 중첩 0 / **KPI**: 무위반 누적 전환 10^6회, 잔류 스캔 커버리지 100% |
| DMA 경로 격리 | SEC-03 | 침해된 Host가 DMA-capable 디바이스를 이용해도 비할당 도메인 메모리에 접근할 수 없다 | **게이트**: 비할당 도메인 접근 0 / **KPI**: DMA 공격 벡터 구현율 100% |
| Workload 무결성 검증 | SEC-04 | 변조/미검증 Workload 이미지는 Host 침해와 독립인 검증 주체에 의해 탑재가 거부된다 | **게이트**: 미검증 이미지 탑재 0건, 변조 이미지 탐지율 100% / **KPI**: 검증 소요 p99, IEC 62443-4-2 EDR 충족률(SL2 100%, SL3 90% 이상) |
| 저장 데이터 기밀성 | SEC-05 | 저장 상태의 영상/모델/추론 데이터는 파일시스템 침해 시에도 평문과 키가 노출되지 않는다 | **게이트**: 파일시스템 침해 시 평문 노출 0건 / **KPI**: 부채널 TVLA t-값 4.5 미만, 키 회전 지연 |
| 보안 채널 신뢰 | SEC-06 | 위조 신원으로는 TEE 호출이 성립하지 않으며 기존 Host→TEE 경로는 무회귀로 유지된다 | **게이트**: 비인가 주체 TEE 호출 성립 0건 / **KPI**: 호출자 신원·무결성 검증 커버리지 100% |
| 침해 탐지/감사증적 | SEC-07 | 침해 시도가 탐지/기록되고 규제(CRA/GDPR) 기한 내 보고 가능한 포렌식 증적이 확보된다 | **KPI**: 비정상 접근 탐지 MTTD, 포렌식 로그 확보 24h 이내, 심각 패치 리드타임(Critical 30일) |
| 실시간 처리 (격리 오버헤드) | PERF-01 | 격리 환경에서도 비격리 대비 유의미한 저하 없이 30fps 실시간 처리를 유지한다 | **KPI**: 비격리 baseline 대비 저하 10% 이내, 30fps 유지, 드롭율 0.1% 이하 |
| 통신 오버헤드 (zero-copy) | PERF-02 | 도메인 간 프레임 전달이 복사 없이 이루어져 실시간성과 전력 예산을 잠식하지 않는다 | **게이트**: 데이터 경로 memcpy 0회 / **KPI**: 전달 지연 p99 5ms, 프레임당 전환 비용 1ms 이하, fps/W 저하 10% 이내 |
| E2E 지연 (캡처→판단) | PERF-03 | 프레임 캡처부터 판단 결과 전달까지 E2E 지연이 실시간 예산 이내다 | **KPI**: p99 100ms 이하, 표준 모델 SingleStream 추론 지연 p90(비격리 대비 상대 성능 90% 이상) |
| HW 주체 전환 지연 | PERF-04 | HW 사용 주체 전환(권한 회수→소거→부여) 지연이 프레임 주기를 잠식하지 않는다 | **KPI**: p99 10ms 이하 (프레임 주기 33ms의 1/3) |
| Glass-to-glass 지연 | PERF-05 | 캡처부터 원격 운영자 화면 표시까지의 지연이 관제/원격 개입 요건을 만족한다 | **KPI**: 캡처→운영자 화면 p95 200ms 이하 |
| 다중 스트림 처리량 | PERF-06 | 다중 카메라 스트림이 동시 유입되어도 드롭 없이 지속 처리한다 | **KPI**: 1080p30 동시 2스트림 이상(4스트림 목표), 채널 지속 처리량 190MB/s 이상 |
| 파이프라인 시작 지연 | PERF-07 | 시작 요청부터 첫 프레임 처리까지 cold start 지연이 기동 요건을 만족한다 | **KPI**: 시작 요청→첫 프레임 cold start p95 (예: 2초 이하) |
| 코어 무수정 수용 | EXT-01 | 신규 Workload를 Framework 코어 수정 없이 패키징/탑재만으로 수용한다 | **게이트**: Framework 코어 수정 0 LoC / **KPI**: breaking change 0건 추세 |
| 이질 런타임 수용 | EXT-02 | 런타임/의존성이 서로 다른 이질 Workload도 동일한 패키지/로딩 계약으로 수용한다 | **KPI**: 이질 파일럿 Workload 수용 3종 이상(C 네이티브/Python 추론/서드파티 바이너리) |
| 온보딩 리드타임 | EXT-03 | 신규 Workload의 개발→통합 온보딩이 표준 절차만으로 단기간에 완료된다 | **KPI**: 신규 Workload 통합 평균 5인일 이내 |
| Fleet 배포 성공률 | EXT-04 | 릴리스가 fleet 전체에 수동 개입 없이 안정적으로 OTA 배포된다 | **KPI**: 릴리스 wave당 98.5% 이상, 수동 개입 없이 |
| 배포 롤백 | EXT-05 | 배포 실패 시에도 fleet 벽돌화 없이 이전 버전으로 신속히 자동 복원된다 | **게이트**: 배포 실패 시 fleet 벽돌화 0건 / **KPI**: 이전 버전 복원 2분 이내·실패 케이스 99.9% 성공 |
| Secure OS/TEE 교체성 | EXT-06 | Secure OS 교체 시 GP 표준 인터페이스 경계 밖의 SW는 수정/재이식하지 않는다 | **게이트**: 인터페이스 외 재이식 파일 0개 / **KPI**: 교체 대응 공수 |
| API 계약 안정성 | EXT-07 | Framework 버전이 릴리스되어도 기존 Workload 자산이 수정 없이 계속 동작한다 | **KPI**: Workload API breaking change 건수/릴리스 0 유지 |
| pVM 장애 격리 | AVL-01 | pVM 장애가 Host, 다른 pVM, 로봇 기본 동작으로 전파되지 않는다 | **게이트**: 장애 전파로 인한 Host/타 pVM 다운타임 0 |
| 장애 복구 시간 (MTTR) | AVL-02 | 장애 pVM이 검출→자원 회수→재기동 절차로 신속히 서비스를 재개한다 | **KPI**: p99 3초 이내 (검출 0.5s / 회수 1s / 재기동 1.5s 분해) |
| 장애 모드 커버리지 | AVL-03 | 장애 카탈로그의 모든 모드가 자동 주입 시험으로 재현/검증 가능하다 | **KPI**: 장애 카탈로그(크래시/hang/부팅실패/OOM/채널소실) 주입 구현율 100% |
| 자원 누수 방지 | AVL-04 | 장애-재시작이 장기 반복되어도 격리 메모리 등 자원 누수가 누적되지 않는다 | **KPI**: 1,000회 crash-restart soak 기준 누수율 0 수렴 |
| 이웃 성능 간섭 | AVL-05 | 한 pVM의 장애 처리/재시작 중에도 정상 pVM의 실시간 성능이 유지된다 | **KPI**: 장애/재시작 중 정상 pVM fps 저하 5% 이내 |
| 서비스 가동률 (SLA) | AVL-06 | 보안 파이프라인 서비스가 상용 운용 SLA 수준의 가동률을 유지한다 | **KPI**: 99.9% 이상/월 (월 다운타임 예산 약 43분) |
| SW 기인 MTBF | AVL-07 | 플랫폼 SW 기인 장애의 평균 무고장 시간이 상용 신뢰성 수준을 만족한다 | **KPI**: 5,000시간 이상 (플랫폼 SW 기인 장애만 계수) |
