...
|---|---|---|---|:---:|---|
| SEC-01 | 기밀성 (Host 침해) | **게이트**: 노출 0건 / **KPI**: 공격 벡터 자동화 커버리지 100%, 경계 돌파 최소 공격 잠재력(CEM) 25점 이상 | pVM 내 canary 마커 주입 후 root Host가 `/proc/kcore`·`/dev/mem`·DMA로 전체 덤프, 마커 검출 자동 판정. 생명주기 13단계(특히 할당/회수)마다 반복 | H/H | QA-01, DP1 |
...
| SEC-04 | Workload 무결성 검증 | **게이트**: 미검증 이미지 탑재 0건, 변조 이미지 탐지율 100% / **KPI**: 검증 소요 p99 | 서명/manifest/버전/해시 변조 케이스를 주입하고 탑재 거부 여부 확인. 검증 주체가 Host 침해와 독립인지 경로 검증 | H/H | DP2 |
...
| PERF-02 | 통신 오버헤드 (zero-copy) | **게이트**: 데이터 경로 memcpy 0회 / **KPI**: 전달 지연 p99 5ms, 프레임당 전환 비용 1ms 이하 | 복사 계열 함수 프로파일 히트 0 확인, dma-buf fence 타임스탬프로 핸드오프 지연, ftrace로 프레임당 hypercall 계측 | H/M | QA-05, DP3 |
| PERF-03 | E2E 지연 (캡처→판단) | **KPI**: p99 100ms 이하 | 캡처 HW 타임스탬프(PTS)를 파이프라인 끝까지 전파, 단계별 지연 분해 | H/H | QA-03 |
