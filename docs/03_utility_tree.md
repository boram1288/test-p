...
> 본 문서는 핵심 품질 속성을 **보안/성능/확장성/강건성** 관점으로 분류하고, 변경 용이성·자원 효율·시험 용이성을 함께 Utility Tree로 구조화하여
...
| 우선순위 | 중요도와 난이도가 같은 경우 보안 → 확장성 → 성능 → 강건성 순으로 평가 |
...
| EXT-01 | 신규 Workload 코어 무수정 수용 | **H** | **H** | 2순위 |
| PERF-01 | E2E 지연 (캡처→판단) | **H** | **H** | 3순위 |
| AVL-01 | pVM 장애 격리 | **H** | **H** | 4순위 |
| SEC-02 | 저장 데이터 기밀성 | **H** | M | 5순위 |
| AVL-02 | 자원 누수 방지 | **H** | M | 6순위 |
| EXT-02 | 온보딩 리드타임 | M | M | 7순위 |
