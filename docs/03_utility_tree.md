...
***[#D5F5E3] PERF-03 통신 오버헤드
****_ 도메인 간 대용량 데이터 전달 시 실시간성을 해치지 않음\n[게이트: dma-buf 전달 시 memcpy 호출 0회]
...
| PERF-03 | 통신 오버헤드 | **H** | M | 5순위 |
| AVL-01 | pVM 장애 격리 | **H** | M | 6순위 |
| AVL-02 | 자원 누수 방지 | **H** | M | 7순위 |
| PERF-02 | Glass-to-glass 지연 | M | M | 8순위 |
| EXT-02 | 온보딩 리드타임 | M | M | 9순위 |
| RES-01 | 메모리/전력 오버헤드 | M | M | 10순위 |
| TST-01 | 자동화 검증 | M | M | 11순위 |
| MOD-01 | Secure OS 교체성 | M | M | 12순위 |
...
| PERF-03 | **게이트**: 도메인 간 dma-buf 버퍼 전달 시 memcpy 호출 0회 | dma-buf 등 버퍼 전달 시 복사 횟수를 계측하고 코드 확인과 PoC 계측 결과를 함께 검증한다. | QS-04 |
...
| PERF-03 | 대용량 데이터 경로에서 복사가 발생하면 zero-copy 목표가 깨지고 실시간성 저하 위험이 커진다. | **불변 조건**. 코드 확인과 PoC 계측으로 검증 |
