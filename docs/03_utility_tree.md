...
| QA-05 | 도메인 간 dma-buf 버퍼 전달 시 memcpy 호출 0회 | dma-buf 등 버퍼 전달 시 복사 횟수를 계측 | 대용량 데이터 전달 경로에서 복사가 발생하면 zero-copy 목표가 깨지고 실시간성 저하 위험이 커짐 | **불변 조건**. 코드 확인과 PoC 계측으로 검증 |
| QA-06 | Workload 서비스 재개 3초 이내, Host/타 pVM 다운타임 0 | 장애 주입 시각부터 Workload 서비스 재개 시각까지 계측하고 타 도메인 영향 여부 확인 | pVM 장애 격리 주장의 최소 조건이며, 재개 시간은 로봇 서비스 허용 공백과 협의 필요 | **불변 조건 + 가정치**. 장애 주입 PoC로 보정 |
