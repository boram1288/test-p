...
| DP-08 | 무결성 보장을 위한 pVM 부팅 방안 설계 | 교차 | QA-01 × FR-06 | DP-03 |
...
- DP-08의 부팅 측정값을 권한 정책과 어떻게 연결할 것인가?
...
- DP-07의 저장 보호와 DP-08의 부팅 무결성은 TrustZone TEE를 신뢰 앵커로 쓸 수 있는지에 따라 구조가 달라진다.
...
- DP-07의 키 관리와 DP-08의 이미지 검증에 TrustZone TEE를 사용할 것인가?
...
**문제 상황**
- Secure Vision AI는 최소 두 개 이상의 pVM을 독립적으로 생성·운용해야 하며, 생명주기 관리 구조가 없으면 후속 HW IP 중재·보안 채널 설계를 배치할 기준이 없다.
