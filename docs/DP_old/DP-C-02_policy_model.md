...
권한은 워크로드 패키지에 포함된 서명된 manifest에 정적으로 선언한다(사용 가능 HW IP, 허용 채널 상대, 메모리 상한 등). 검증은 TEE 앵커(DP-C-04 이미지 검증과 같은 서명 체인)가 수행하고, 집행은 권한이 실제로 "능력(capability)"으로 부여되는 길목 — pVM 생성, 메모리 공유, SMMU 매핑, 채널 수립 — 에서 TCB 메커니즘이 게이트한다. Host 프레임워크는 정책의 운반자/요청자일 뿐 결정자가 아니다.
...
- **철회**: pVM 종료/워크로드 삭제 시 생명주기 관리(DP-C-03)가 자원 회수, TEE가 키 방출 중단
...
| G5 | **measured 권한 바인딩**: manifest 권한은 DP-C-04 부팅 측정값에 바인딩되어야 "승인된 워크로드"와 "Host가 만든 임의 인스턴스"를 구분할 수 있다 (DP-C-05 G2와 동일 근원). **[역반영 1단계 완료 — 2026-06-12, 14 문서 5.3절]** 바인딩 참조 필드 = DP-C-04 측정값 포맷의 manifest hash/manifest version(anti-rollback)으로 확정. **완전 폐쇄는 H7 HV 확인 후**(측정 주체/시점 실증) | DP-C-04 G3, H7 |
