...
**산출근거 Driver (05 DP-C-02)**: QA-01, QA-02, FR-01(pVM 생성 권한)/FR-02(다중 pVM 권한 분리), FR-03, FR-04, FR-06, CS-02

### 1.1 설계질문 (05 문서 DP-C-02)

- pVM 생성, HW IP 사용, 채널 수립, Workload 로딩 권한은 누가 정의/검증/집행하는가?
- 권한 정책은 Workload 패키지 manifest, 도메인별 권한 테이블, 런타임 정책 중 무엇으로 표현하는가?
- EL2, TrustZone TEE, 별도 서비스 pVM, Host Framework 중 어떤 컴포넌트가 어떤 권한을 집행하는가?
- 권한 불일치, 정책 위반, 권한 철회 시 pVM/채널/HW IP 할당을 어떻게 차단/회수하는가?
- DP-C-04의 패키지 manifest 권한과 채널 상대 인증의 pVM 아이덴티티를 어떻게 연결할 것인가?

### 1.2 신뢰 모델 전제 (`06_dpc01_trust_model.md` 6.5절)

- Host Framework의 단독 권한 집행은 불가하다. 집행 지점은 TCB 요소(EL2 기 제공 메커니즘, TEE, 승인된 서비스 pVM) 중에서 선택한다.
...
권한은 Workload 패키지에 포함된 서명된 manifest에 정적으로 선언한다(사용 가능 HW IP, 허용 채널 상대, 메모리 상한 등). 검증은 TEE 앵커(DP-C-04 이미지 검증과 같은 서명 체인)가 수행하고, 집행은 권한이 실제로 "능력(capability)"으로 부여되는 길목 — pVM 생성, 메모리 공유, SMMU 매핑, 채널 수립 — 에서 TCB 메커니즘이 게이트한다. Host Framework는 정책의 운반자/요청자일 뿐 결정자가 아니다.

- **정의**: 배포 시점, manifest 서명(Workload 발급 키 체계)
- **검증**: 로딩 시점, TEE 서명 검증 (DP-C-04와 동일 체인)
- **집행**: 능력 부여 시점, EL2 기 제공 메커니즘(메모리 donate/share, Stage-2)과 SMMU 설정 경로, TEE(키 방출 거부)
- **철회**: pVM 종료/Workload 삭제 시 생명주기 관리(DP-C-03)가 자원 회수, TEE가 키 방출 중단
- **장점**: 신뢰 모델과 완전 정합(Host는 결정자가 아님), TCB 추가 없음, 정적이라 검증/증빙(CS-04)이 쉬움
...
- **단점**: 정책 pVM이 모든 권한 결정의 단일 신뢰 지점이 되어 TCB 편입이 필수(부트스트랩/자원 비용/DP-C-01 조건부 규칙). 모든 권한 판단 경로에 pVM 왕복이 끼어들어 지연이 늘고(QA-09), DP-C-03/C-06보다 먼저 완성되어야 하는 순서 부담이 크다.

### 후보 C: Host Framework 집행 + TEE 사후 감사

Host Framework가 권한을 판단/집행하되, 모든 결정을 보안 이벤트 로그로 남겨 사후 감사한다.
...
| E4. 집행 일관성 | DP-C-03/C-06가 같은 권한 가정 위에서 설계될 수 있는가 |
| E5. 성능 | 권한 판단이 데이터 경로/제어 경로에 주는 지연 (QA-09) |
| E6. 증빙 적합성 | 권한 체계 자체를 CS-04 증빙에 포함할 수 있는가 |
...
| E4 집행 일관성 | 상 — 단일 manifest가 DP-C-03/C-06의 공통 기준 | 중 — 모든 권한 경로의 정책 pVM 경유 강제 필요 | 하 — 우회 시 사후 감사만 남음 |
| E5 성능 | 상 — 판단이 생성/부여 시점에 종료, 데이터 경로 지연 없음 | 중 — 런타임 질의 왕복이 전환/수립 지연 증가 | 상 — 단 요구 충족과 무관 |
| E6 증빙 적합성 | 상 — 서명 manifest와 게이트 결과는 CS-04 증빙에 적합 | 중 — 동적 상태의 결정 재현성 곤란 | 하 — 사전 차단 증빙 불가 |
...
| G2 | **manifest 보안 속성 정의**: 스키마/버전/서명 키 체계/폐기/회전/롤백 방지/측정값↔권한 바인딩을 정의한다. 스키마/패키징은 DP-C-04, 측정값 바인딩은 DP-C-04, 아이덴티티 연결은 채널 상대 인증과 분담 | DP-C-04, DP-C-04, 채널 상대 인증 |
...
| G5 | **measured 권한 바인딩**: manifest 권한은 DP-C-04 부팅 측정값에 바인딩되어야 "승인된 Workload"와 "Host가 만든 임의 인스턴스"를 구분할 수 있다 (DP-C-05 G2와 동일 근원). **[역반영 1단계 완료 — 2026-06-12, 14 문서 5.3절]** 바인딩 참조 필드 = DP-C-04 측정값 포맷의 manifest hash/manifest version(anti-rollback)으로 확정. **완전 폐쇄는 H7 HV 확인 후**(측정 주체/시점 실증) | DP-C-04 G3, H7 |

### 능력 부여 길목별 집행 주체 매핑표 (초안 — G1 검증 대상)

| 길목 | 1차 게이트 후보 | 비고 |
|------|----------------|------|
| pVM 생성/이미지 로딩 | TEE (이미지/manifest 서명 검증, DP-C-04) | 검증 실패 시 키 방출 거부로 실행 차단 |
...
- DP-C-03/C-06은 "권한 = 서명된 manifest의 정적 선언, 집행 = TCB 게이트 fail-closed"를 공통 전제로 설계한다.
- Host Framework는 정책의 운반자/요청자이며 어떤 길목에서도 단독 결정자가 아니다.
- 키 방출 게이트(TEE)는 모든 Workload 보호 자산의 최종 방어선이다 — DP-C-08 설계의 출발 전제.
