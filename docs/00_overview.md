...
단, Camera/AI HW 등 HW IP는 보안 파이프라인 전용이 아니다. Host의 일반 기능(일반 촬영, 일반 AI 추론 등)도 동일한 HW IP를 사용하므로, 보안 파이프라인이 HW IP를 독점해서는 안 되며 **Host와 도메인이 HW IP를 동시에 사용**할 수 있어야 한다.
...
| R-2 | **HW 고성능 연산 지원** | 실시간 영상 처리와 AI 추론을 위해 도메인에서 Camera/AI HW 등 HW IP 가속을 안전하게 사용할 수 있어야 함 (SW 처리만으로는 실시간성/연산량 미충족). HW IP는 Host의 일반 기능도 함께 사용하는 공유 자원이므로, 도메인 독점 할당이 아니라 Host와 도메인의 동시 사용을 지원해야 함 |
