...
- **CS-06**: Secure OS 신규 개발은 범위 밖이다 — 기존 Secure OS의 이식/수정 부담을 최소화하는 표준화 수준이어야 한다.
...
- **장점**: Framework 측 수용 코드가 단일 경로라 QA-05(무수정 수용)이 가장 강하게 성립한다. 검증 지점/채널 규약이 균일해 DP-09/DP-02 연결이 단순하다.
- **단점**: 기존 Secure OS는 부팅 규약/디바이스 모델이 제각각이라 표준 ABI에 맞추는 이식 수정량이 커진다 — CS-06(이식/수정만 허용)의 취지인 "이식 부담 최소화"와 긴장. Secure OS 커널 구조에 따라 표준 준수가 불가능한 경우 수용 자체가 막힌다(FR-07 위험).
...
- **장점**: FR-06(동적 탑재)과 FR-07(Secure OS 이식)을 각 프로파일이 분담해 모두 수용한다. P1은 QA-05 무수정 수용, P2는 이식 수정량 최소(CS-06 정합). 검증/manifest/암호화 레이아웃은 단일이라 DP-09/DP-02/DP-07 연결 지점이 하나다.
...
- **단점**: Workload마다 VMM 구성/채널 처리 코드가 추가된다 — 신규 Workload 수용이 "Framework 수정 없이"(QA-05) 성립하지 않을 위험. 검증 지점/채널 규약이 파편화되어 DP-09 검증과 DP-02 집행의 균일성이 깨진다. Secure OS 교체 시 무관 SW 무수정(QA-06)을 보장할 경계가 없다.
...
- **Secure OS 교체 경계(QA-06)**: 교체 가능 단위 = 패키지. Framework/Host Middleware/타 Workload가 보는 것은 manifest 선언과 채널 규약뿐 — 패키지 내부 변경이 경계 밖으로 새지 않는다.
...
| 기준 | 설명 |
|------|------|
| E1. 동적 확장성 | 신규 Workload를 Framework 수정 없이 수용하는가 (FR-06, QA-05) |
| E2. Secure OS 이식 적합성 | 기존 Secure OS의 이식 수정량이 CS-06 취지에 맞게 최소인가 (FR-07) |
| E3. 교체 격리 | Secure OS 교체 시 무관 SW 무수정이 구조적으로 보장되는가 (QA-06) |
...
|------|---------------|------------------|-------------|-------------|-------------|----------|
| A 단일 표준 ABI | 상(조건부) | **하** — 기존 Secure OS 수정량이 커 CS-06과 긴장 | 상 | 중 | 상 | 중 — 이식팀이 ABI에 강하게 종속 |
...
| C 최소 표준 | **하** — QA-05 약함 | 상 | **하** — 교체 경계가 VMM 구성까지 번짐 | **하** — 집행/검증 경로 파편화 | **하** | **하** |

### 3.2 종합 판정

- **A**: 보안/검증 단순성은 좋지만 Secure OS 수용성이 낮아 기준안 부적합.
- **B: 조건부 채택 대상.** 공통 포맷을 유지하면서 P1/P2의 현실적 차이를 흡수.
- **C**: 비교 기준선으로만 의미. QA-05/QA-06/DP-02 정합을 구조적으로 보장 못함.
...
1. **Codex 등급/근거 타당** — A/B/C 상대 서열과 채택 후보 선택에 전제 문서와의 모순 없음. 후보 A E2 "하"의 근거인 CS-06은 원문이 "신규 개발 제외 + 이식/수정만"이며 "이식량 최소화"는 취지 해석임을 명시한다.
...
- 교체 단위 = 패키지. Framework/Host Middleware/타 Workload가 보는 것은 manifest 선언과 채널 규약뿐이다(QA-06 경계).
