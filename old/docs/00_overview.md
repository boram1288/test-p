...
| **HW IP의 REE 종속** | Camera/AI HW 등 HW 가속 IP가 REE 영역에서만 사용 가능하여 도메인에서 활용할 방법이 없음 |
...
1) Multi VM 실행 (메모리 격리)
2) Data 저장/복구시  ENC/DEC 진행
3) 추론 결과 Host APP에서 출력
...
| **기존 구조 한계 노출성** | 다중 도메인, 고성능 연산, HW IP 접근을 동시에 요구하여 2절의 기존 구조(특히 TrustZone 이진 격리)의 한계가 가장 분명하게 드러남 |
...
단, Camera/AI HW 등 HW IP는 보안 파이프라인 전용이 아니다. Host의 일반 기능(일반 촬영, 일반 AI 추론 등)도 동일한 HW IP를 사용한다. 단일 Context HW에는 한 시점에 하나의 주체만 접근 권한을 갖게 하되, 배타적 고속 전환으로 Host와 도메인의 동시 사용 요구를 시분할 수용해야 한다. 여기서 동시 사용은 물리적으로 같은 순간의 병렬 접근이 아니라 두 주체의 요청을 서비스 예산 안에서 함께 처리하는 논리적 동시성을 뜻한다.
...
| R-2 | **HW 고성능 연산 지원** | 실시간 영상 처리와 AI 추론을 위해 도메인에서 Camera/AI HW 등 HW IP 가속을 안전하게 사용할 수 있어야 함 (SW 처리만으로는 실시간성/연산량 미충족). 단일 Context HW는 한 시점에 한 주체에만 배타적으로 할당하고, Host와 도메인의 요청은 고속 전환으로 시분할 수용해야 함 |
| R-3 | **다중 도메인 동시 운용** | Secure Camera 도메인과 Secure AI 도메인을 독립적으로 동시에 생성/운용해야 함 |
...
| **포함** | 기존 Secure OS를 pVM 도메인에서 동작/확장시키기 위한 SW 이식/수정 |
